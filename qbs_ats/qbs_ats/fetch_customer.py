...
def daily_fetch_crm_opportunities_and_sync():
    ...
    opp_fields = [
        "name", "customer_name", "party_name", "custom_vertical", "status",
        "country", "state", "city", "custom_actual_revenue", "custom_open_mandates",
        "custom_requirement", "custom_closed_mandates", "custom_salary_range",
        "opportunity_owner", "opportunity_from", "source", "custom_client_status",
        "probability", "custom_expected_revenue_yearly", "industry", "transaction_date"
    ]
    ...
    try:
        ...
        for opp in opportunities:
            ...
            if frappe.db.exists("Client", {"opportunity_id": crm_opp_id}):
                continue # Agar ID pehle se hai, toh skip karo (No Update)

            try:
                # Sirf naya record create hoga
                client = frappe.new_doc("Client")
                client.opportunity_id = crm_opp_id

                # --- DATA MAPPING ---
                client.opportunity_name = opp.get("customer_name")
                ...
                frappe.log_error(frappe.get_traceback(), f"Client Sync Error: {crm_opp_id}")

        frappe.db.commit()

    except Exception:
        frappe.log_error(frappe.get_traceback(), "Client Sync Fatal Error")
